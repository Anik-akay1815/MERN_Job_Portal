...
function Icon({ name, size = 17 }) {
  const paths = {
    ...
    alert: <><path d="m12 3 9 18H3L12 3Z" /><path d="M12 9v4M12 17h.01" /></>,
  };
...
function JobDetail() {
  ...
  if (!job) {
    return (
      <>
        <Navbar />
        <h2 className="text-center mt-20 text-xl font-semibold text-slate-700 dark:text-slate-200">Loading...</h2>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-linear-to-br from-[#f5f6fa] via-[#f1efff] to-[#eef2ff] dark:bg-[#0f1420] dark:bg-none px-5 py-10 relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto bg-white/80 dark:bg-[#161c2e]/80 backdrop-blur-xl border border-white/70 dark:border-white/10 rounded-3xl shadow-2xl dark:shadow-black/40 overflow-hidden"><div className="h-1 bg-linear-to-r from-blue-500 via-indigo-500 to-purple-500" />
          {/* Job Header */}
          <div className="px-7 sm:px-9 pt-7 pb-6 border-b border-slate-200/70 dark:border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-5"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400 mb-2">Job Opportunity</p><h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">{job.title}</h1>

            <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 mt-2">Posted by <Link to={`/company/${job.company._id}`} className="text-blue-600 dark:text-blue-300 font-semibold hover:text-indigo-600 dark:hover:text-indigo-200 transition-colors">{job.company.companyname}</Link></p></div><div className="hidden sm:flex w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-400/20 items-center justify-center text-blue-600 dark:text-blue-300"><Icon name="briefcase" size={22} /></div></div>
          </div>
          ...
          <div className="px-7 sm:px-9 py-5 border-y border-slate-200/70 dark:border-white/10 bg-slate-50/50 dark:bg-white/2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">Interested in this role?</p><p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Submit your application to get started.</p></div>
            <button
          ...
          <div className="px-7 sm:px-9 pt-7 mb-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full bg-linear-to-b from-blue-500 to-purple-500" />Job Description
            </h2>
          ...
          {job.skillsRequired && job.skillsRequired.length > 0 && (
            <div className="px-7 sm:px-9 mb-8">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full bg-linear-to-b from-purple-500 to-indigo-500" />Required Skills
              </h2>
          ...
          <div className="px-7 sm:px-9 pb-9">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full bg-linear-to-b from-indigo-500 to-blue-500" />Job Information
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Number of Openings */}
              <div className="bg-slate-50/70 dark:bg-white/5 border border-slate-200/70 dark:border-white/10 rounded-xl p-4 hover:border-blue-200 dark:hover:border-blue-400/20 transition-colors">
                <p className="text-sm text-slate-500 dark:text-slate-400">Number of Openings</p>
      ...
      {notice && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white/95 dark:bg-[#161c2e]/95 backdrop-blur-xl rounded-2xl shadow-2xl p-6 max-w-sm w-full border border-white/70 dark:border-white/10">
            ...
            >
              {notice.title === "Success" ? <Icon name="check" size={22} /> : <Icon name="alert" size={22} />}
            </div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white text-center mb-2">
              {notice.title}
            </h3>
